...
for klient in klienti:
    os.system("cls")
    print u"Vitajte!\n\nExtrahujem faktúry: %s\n\n[%s%s] %d/%d strán ostáva.\n\n" % (unicode(klient, "ascii", "ignore").encode("ascii"), "#"*int(50*(klienti.index(klient)/float(pocet_klientov)))," "*int(50*((pocet_klientov-klienti.index(klient))/float(pocet_klientov))),pocet_stran_fakturacie - pocet_stran_rozhodenych, pocet_stran_fakturacie)
...
if pocet_stran_rozhodenych == pocet_stran_fakturacie:
    ...
    print u"POZOR! Krížová kontrola neprebehla úspešne, počet strán sa líši!"
